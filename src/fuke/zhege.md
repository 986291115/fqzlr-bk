...
默认值：
className	字符串	—	容器的额外 CSS 类。
图片	字符串数组	[]	用于显示的图片 URL 数组。
containerWidth	数字	400	容器的宽度（像素）。
containerHeight	数字	400	容器的高度（像素）。
动画延迟	数字	0.5	动画开始前的延迟时间（秒）。
animationStagger	数字	0.06	每张卡片动画之间的时间间隔（秒）。
缓动类型	字符串	弹性.out(1, 0.8)	弹跳的缓动函数。
transformStyles	string[]	各种旋转/平移	每个卡片位置的定制变换。
enableHover	布尔值	false	如果为 true，鼠标悬停会推开兄弟元素并使悬停卡片的旋转扁平化。